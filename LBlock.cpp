...
LBlock::LBlock(int id, Board* b, int lv, Displays* ob):Block('L', id, b, lv, vector<Point>{Point{2,2}, Point{2,3}, Point{1,3}, Point{0,3}}, Point{2,3}, 0, ob){}
...
void LBlock::clockwise(){
    ...
    if(getState() == 0){
        for(int i = 0; i < getPos().size(); i++)
        {
            ...
            if(!check(new_x - 2, new_y, getBoard()))
            {
                canRotate = false;
                break;
            }
        ...
        if(canRotate)
        {
            ...
            for(int i = 0; i < getPos().size(); i++)
            {
                int new_x = getRotatePoint().x - getPos()[i].y + getRotatePoint().y;
                int new_y = getRotatePoint().y + getPos()[i].x - getRotatePoint().x;
                getPos()[i] = Point{new_x - 2, new_y};
            }
            setState(1);
            Point new_rotate = Point{getRotatePoint().x - 2, getRotatePoint().y};
        ...
        for(int i = 0; i < getPos().size(); i++)
        {
            ...
            int new_x = getRotatePoint().x - getPos()[i].y + getRotatePoint().y;
            int new_y = getRotatePoint().y + getPos()[i].x - getRotatePoint().x;
            if(!check(new_x + 1, new_y + 2, getBoard()))
            {
                canRotate = false;
                break;
            }
        ...
        if(canRotate)
        {
            ...
            for(int i = 0; i < getPos().size(); i++)
            {
                int new_x = getRotatePoint().x - getPos()[i].y + getRotatePoint().y;
                int new_y = getRotatePoint().y + getPos()[i].x - getRotatePoint().x;
                getPos()[i] = Point{new_x + 1, new_y + 2};
            }
            setState(0);
            Point new_rotate = Point{getRotatePoint().x + 1, getRotatePoint().y + 2};
            setRotatePoint(new_rotate);
            checkHeavy();
            draw(getType());
        }
